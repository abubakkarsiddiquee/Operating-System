...
struct Process {
    ...
    int original_index; // To track back to original input order
};
    ...
    while (completed_processes < n && current_time < total_timeline_length + 1) {
        ...
        for (int i = 0; i < n; ++i) {
            if (processes[i].arrival_time <= current_time && processes[i].remaining_time > 0) {
                ...
                // Tie-breaker: If remaining times are equal, use FCFS (earlier arrival)
                else if (processes[i].remaining_time == shortest_remaining_time && selected_process_index != -1) {
