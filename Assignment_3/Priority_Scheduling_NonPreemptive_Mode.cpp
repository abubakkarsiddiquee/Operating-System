...
struct Process {
    ...
    int remaining_time; // Used for tracking completion
...
// Main scheduling logic (Non-Preemptive Priority)
    ...
    while (completed_processes < n) {
            ...
            if (processes[i].arrival_time <= current_time && processes[i].remaining_time > 0) {
                ...
                // Tie-breaker: If priorities are equal, use FCFS (earlier arrival)
                    ...
                    if (selected_process_index == -1 || 
                        processes[i].arrival_time < processes[selected_process_index].arrival_time) 
                    {
                        selected_process_index = i;
                    }
                }
        ...
        if (selected_process_index == -1) {
            ...
            if (found_next && next_arrival != INT_MAX) {
                ...
                current_time++; // Should only run if there are no more waiting processes
            }
            continue;
        }
        ...
        // Non-Preemptive: Execute the selected process fully
        current_p.completion_time = start_time + current_p.burst_time;
        current_p.response_time = start_time - current_p.arrival_time;
        current_p.remaining_time = 0; // Mark as completed
